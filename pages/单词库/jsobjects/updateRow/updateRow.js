export default {
	...
	async updateRes(){
		...
		GenResource.run({course_number,level,course,scenes,cards,skip_audio:true})
	},
	...
	async updateJsonImage(updateVal, newName,ismodify){
		...
		function _updateScrits(scripts, on, nn){
			console.log(scripts,'7878/')
			// if(!scripts?.length)
			// return

			var val = scripts.scripts
			if(scripts.word_metadata.basic_info &&scripts.word_metadata.basic_info.image == on){
				scripts.word_metadata.basic_info.image = nn
			}

			if(scripts.word_metadata.examples){
				Object.values(scripts.word_metadata.examples).forEach(exampleArray => {
	...
	async updateJsonScene(updateVal, newName,ismodify){
		...
		await GenResource.run({course_number,level,course,skip_audio:true}).then(async res=>{
			if(res.scripts == 'success'){
				showAlert('保存成功','success')
				// await updateRow.update()
				if(ismodify){
