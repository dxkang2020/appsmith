		...
		await getScenesPrompt.run({names}).then(res=>{
			...
			if(names.length !== res.length){
				...
				var urlarr = res.map((v)=>{
					return {
						...v,
						urls :`https://af.runfox.cn/courses/scenes/${v.name.toLowerCase()}.webp?r=${Math.random()}` 
					}
				})
