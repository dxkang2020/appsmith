export default {
	...
	// 生成图片事件
	async getAIIMG(currentIndex,item, img){
		console.log(currentIndex,item,'45')
		// this.imgItem = item
		let params = {
			prompt:item.clip
		}
		await Api4.run(params).then((res)=>{

			item.ImgURL = 'data:image/png;base64,' + res
			// 更新数据源中对应 item 的 imageUrl 属性
			const updatedListData = [...Table1.selectedRow.scenes_prompt];
			updatedListData[currentIndex].imageUrl = item.ImgURL

			Table1.selectedRow.scenes_prompt = updatedListData
			// img.setImage(item.ImgURL)
